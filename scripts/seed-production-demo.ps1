...
Write-Host "Seeding FreshTrace demo users and demo workflows into production..."
npx.cmd deno run --allow-env --allow-net scripts/seed-demo-data.ts `
  $supabaseUrl `
  $anonKey `
  $ServiceRoleKey
...
if ($exitCode -ne 0) {
  throw "Production demo data generation failed with exit code $exitCode."
}

Write-Host "Production demo data is ready."

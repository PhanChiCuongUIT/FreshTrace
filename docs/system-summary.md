...
| --- | --- | --- | --- | --- |
| Authentication and profile | Customer, Employee, Manager, Admin | Login, register, profile, avatar upload, password reset/change; inactive/banned accounts are blocked with a clear message | Supabase Auth, `account-status`, profile update through PostgREST, Cloudinary upload helper for avatar, Auth ban sync from Admin governance | `auth.users`, `users`, `roles`, `coupons` signup trigger |
...
| Product management | Manager | Catalog > Products drawer create/edit, local image preview, image upload only after Save | Cloudinary signed upload helper; PostgREST product write and cache refresh | `products`, `suppliers`, `categories` |
...
| Inventory control | Manager | Catalog > Inventory drawer records stock count/import/export/reserve/release corrections and confirmation | Audited stock adjustment RPC/table writes guarded by RLS/business constraints | `inventory`, `inventory_transactions`, `batches` |
...
| Fresh Rescue | Customer, Manager | Customer Home/Rescue lists near-expiry offers; Manager Catalog > Rescue drawer creates/edits discounted price with read-only original price | Product search RPC merges rescue price and discount; DB copies original price from active Prices row and validates near-expiry/stock/price rules | `fresh_rescue_deals`, `batches`, `inventory`, `prices` |
...
| Coupon promotion | Customer | Profile tabs show reward progress, available coupons, transactions, policies and customer guidance | DB triggers issue welcome, delivered-order, spending, approved-report and random large-order rewards; cancellation RPC restores/issues eligible credit; successful use removes the coupon | `coupons`, `orders`, `deliveries`, `reports`, `payments` |
...
| COD/PayOS settlement | Customer, Employee | Customer selects COD; shipper screen supports cash collection or customer scans shipper PayOS QR; payment success screen can sync payOS when webhook is unavailable | PayOS request creation; webhook or `sync-payos-payment` confirms paid state; cash collection requires remittance before delivery close | `payments`, `payos_requests`, `delivery_payment_collections`, `order_tracking` |
...
| Cancellation and refund coupon | Customer | Customer can cancel only pending orders; paid pending order returns full-value coupon | `cancel-order` checks payOS first, confirms already-paid links if needed, then calls hardened cancellation RPC | `orders`, `payments`, `inventory`, `coupons`, `order_tracking`, `payos_requests` |
...
| Chat | Customer, Manager, Employee, Admin | Messenger-like chat, avatar grouping, mobile reactions, attachments, share product/order; Customer-Manager rooms are one per customer/manager pair | `create_chat_room`, `list_my_chat_rooms`, Cloudinary file upload, reaction upsert/update | `chat_rooms`, `chat_room_members`, `chat_messages`, `chat_message_reactions` |
| Auto chat after order | Customer, Manager, Employee | Order reuses or creates the customer-manager pair chat; assignment creates customer-shipper/manager-shipper order chats | DB trigger and `assign-delivery` function ensure rooms/members | `order_manager_assignments`, `chat_rooms`, `chat_room_members` |

...
export function ManagerCatalogPage() {
  ...
  const saveRescue = async () => {
    const originalPrice = Number(rescueForm.originalPrice)
    ...
    if (!Number.isFinite(originalPrice) || !Number.isFinite(rescuePrice) || originalPrice <= 0 || rescuePrice <= 0) return feedback.error('Original and rescue prices must be greater than zero')
    if (rescuePrice >= originalPrice) return feedback.error('Fresh Rescue price must be lower than the original price')

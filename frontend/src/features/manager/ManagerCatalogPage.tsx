...
export function ManagerCatalogPage() {
  ...
  const adjustInventory = async () => {
    ...
    const result = await supabase.rpc('adjust_inventory', { p_batch_id: inventoryForm.batchId, p_new_quantity: Number(inventoryForm.quantity), p_note: inventoryForm.note.trim() || 'Manager inventory adjustment' })
    if (result.error) return feedback.error(result.error.message)
  ...
  return <div>
    ...
    <div className="card mt-4 grid gap-3 p-4 md:grid-cols-[1fr_220px]"><label className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-black/35" size={18}/><input className="input" style={{ paddingLeft: '2.75rem' }} value={query} onChange={event => setQuery(event.target.value)} placeholder={`Search ${section}`}/></label><select className="input" value={status} onChange={event => setStatus(event.target.value)}><option value="">All statuses</option>{['active','inactive','pending','approved','rejected','available','locked','expired','sold_out','near_expiry'].map(value => <option key={value}>{value}</option>)}</select></div>

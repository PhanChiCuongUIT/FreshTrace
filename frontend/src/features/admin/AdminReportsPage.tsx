...
type Report = { report_id: string; type: string; description: string; status: string; response: string | null; created_at: string; reporter: { name: string; email: string }; reported_user: { name: string; email: string } | null; orders: { order_code: number } | null }
...
export function AdminReportsPage() {
  ...
  const reports = useQuery({ queryKey: ['admin-reports'], queryFn: async () => {
    const result = await supabase.from('reports').select('report_id,type,description,status,response,created_at,reporter:users!reports_user_id_fkey(name,email),reported_user:users!reports_reported_user_id_fkey(name,email),orders(order_code)').order('created_at', { ascending: false })
  ...
  return <div><PageHeader eyebrow="Moderation" title="Reports and supplier approval" />
    ...
    <h2 className="mb-3 mt-7 text-xl font-black">User reports</h2><div className="space-y-3">{reports.data?.map(item => <article key={item.report_id} className="card p-5"><div className="flex flex-wrap justify-between gap-2"><div><b className="capitalize">{item.type.replaceAll('_', ' ')}</b><p className="text-sm text-black/50">Reporter: {item.reporter.name} / {item.reporter.email}</p><p className="text-sm text-black/50">{item.reported_user ? `Reported user: ${item.reported_user.name} / ${item.reported_user.email}` : item.orders ? `Order #${item.orders.order_code}` : 'General report'} / {dateTime(item.created_at)}</p></div><Badge tone={item.status === 'resolved' ? 'green' : item.status === 'rejected' ? 'red' : 'orange'}>{item.status}</Badge></div><p className="my-4">{item.description}</p>{['pending','processing'].includes(item.status) ? <div className="flex flex-col gap-2 sm:flex-row"><input className="input" placeholder="Admin response" value={responses[item.report_id] ?? ''} onChange={event => setResponses({ ...responses, [item.report_id]: event.target.value })}/><button className="btn-primary" onClick={() => resolve(item.report_id, 'resolved')}>Resolve</button><button className="btn-secondary text-red-600" onClick={() => resolve(item.report_id, 'rejected')}>Reject</button></div> : <p className="text-sm text-black/55">Response: {item.response ?? 'No response'}</p>}</article>)}</div>
  </div>
}

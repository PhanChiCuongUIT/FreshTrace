...
export function AuthConfirmPage() {
  ...
  useEffect(() => {
    ...
    if (!tokenHash) {
      setError('This confirmation link is missing its verification token.')
      return
    }
  ...
  return <div className="grid min-h-screen place-items-center bg-[radial-gradient(circle_at_top_left,#d8fbe4,transparent_45%),#f7f8f3] p-4">
    <section className="card w-full max-w-md p-8 text-center">
      ...
      <p className={`mt-3 text-sm ${error ? 'text-red-600' : 'text-black/55'}`}>{error || message}</p>
      {error && <Link className="btn-primary mt-6 w-full" to="/login">Back to sign in</Link>}
    </section>
  </div>
}

...
import { Camera, CheckCircle2, MapPin, Phone, QrCode, TriangleAlert, Upload } from 'lucide-react'
...
export function ShipperDashboard() {
  ...
  const deliveries = useQuery({ queryKey: ['shipper-deliveries', profile?.user_id], enabled: Boolean(profile?.user_id), queryFn: async () => {
    const result = await supabase.from('deliveries').select('delivery_id,status,proof_image_url,delivery_batch_checks(batch_id,matched,checked_at),delivery_payment_collections(method,status,remittance_status),orders(order_id,order_code,delivery_address,users(name,phone),payments(method,status),order_items(order_item_id,product_name,quantity,batches(batch_id,batch_code)))').eq('employee_id', profile!.user_id).order('created_at', { ascending: false })
  ...
  const verifyScanned = async (delivery: Delivery, value: string) => {
    ...
    const matched = order?.order_items.find(item => { const batch = one(item.batches); return parsed.batchId ? batch?.batch_id === parsed.batchId : batch?.batch_code === parsed.batchCode })
  ...
  const updateStatus = async (delivery: Delivery, status: 'picked_up' | 'delivering' | 'delivered' | 'failed') => {
    ...
    if (!approved) return
    try {
      await callFunction('update-delivery-status', { deliveryId: delivery.delivery_id, status, note, proofImageUrl: status === 'delivered' ? (proofs[delivery.delivery_id] ?? delivery.proof_image_url) : undefined })
  ...
  const uploadProof = async (deliveryId: string, file?: File) => {
    if (!file) return
    setUploading(deliveryId)
    try {
      const imageUrl = await uploadImage(file, 'deliveries')
      setProofs(current => ({ ...current, [deliveryId]: imageUrl }))
    } catch (error) { toast(messageOf(error), 'error') } finally { setUploading('') }
  }
  ...
  return <div className="mx-auto max-w-xl lg:max-w-4xl"><PageHeader eyebrow="Mobile delivery" title="My deliveries" /><div className="mt-5 space-y-4">{!deliveries.data?.length ? <EmptyState title="No deliveries assigned" /> : deliveries.data.map(delivery => { const order = one(delivery.orders); const customer = one(order?.users); const payment = one(order?.payments); const collection = delivery.delivery_payment_collections[0]; const proofUrl = proofs[delivery.delivery_id] ?? delivery.proof_image_url; const allVerified = Boolean(order?.order_items.length) && order!.order_items.every(item => { const batch = one(item.batches); return batch && delivery.delivery_batch_checks.some(check => check.batch_id === batch.batch_id && check.matched) }); return <article key={delivery.delivery_id} className="card overflow-hidden">

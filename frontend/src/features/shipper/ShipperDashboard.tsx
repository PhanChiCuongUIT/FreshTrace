...
import { CheckCircle2, MapPin, Phone, QrCode, Search, TriangleAlert } from 'lucide-react'
...
export function ShipperDashboard() {
  ...
  const updateStatus = async (delivery: Delivery, status: 'picked_up' | 'delivering' | 'delivered' | 'failed') => {
    const note = status === 'failed' ? await askText({ title: 'Mark delivery failed?', description: 'Enter the reason so the manager can recover or reassign this order.', placeholder: 'Failure reason', confirmLabel: 'Mark failed', danger: true, required: true }) : undefined
  ...
  const filtered = deliveries.data?.filter(delivery => {
    const order = one(delivery.orders)
    const customer = one(order?.users)
    const items = many(order?.order_items)
    const searchable = `${order?.order_code ?? ''} ${order?.delivery_address ?? ''} ${customer?.name ?? ''} ${customer?.phone ?? ''} ${items.map(item => `${item.product_name} ${one(item.batches)?.batch_code ?? ''}`).join(' ')}`.toLowerCase()
    return (!statusFilter || delivery.status === statusFilter) && (!query || searchable.includes(query.toLowerCase()))
  })
  return <div className="mx-auto max-w-xl lg:max-w-4xl"><PageHeader eyebrow="Mobile delivery" title="My deliveries" /><div className="card mt-5 space-y-3 p-3"><label className="relative block"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-black/35" size={18}/><input className="input" style={{ paddingLeft: '2.75rem' }} value={query} onChange={event => setQuery(event.target.value)} placeholder="Search order, customer, address, batch"/></label><div className="flex gap-2 overflow-x-auto pb-1">{statuses.map(status => <button key={status || 'all'} type="button" onClick={() => setStatusFilter(status)} className={`shrink-0 rounded-full px-4 py-2 text-sm font-black capitalize ${statusFilter === status ? 'bg-brand-600 text-white' : 'bg-black/[0.05] text-black/60'}`}>{status ? status.replaceAll('_', ' ') : 'All'}</button>)}</div></div><div className="mt-5 space-y-4">{!filtered?.length ? <EmptyState title="No matching deliveries" /> : filtered.map(delivery => { const order = one(delivery.orders); const customer = one(order?.users); const payment = one(order?.payments); const collections = many(delivery.delivery_payment_collections); const checks = many(delivery.delivery_batch_checks); const items = many(order?.order_items); const collection = collections[0]; const allVerified = items.length > 0 && items.every(item => { const batch = one(item.batches); return Boolean(batch && checks.some(check => check.batch_id === batch.batch_id && check.matched)) }); const codReady = payment?.method !== 'cod' || Boolean(collection && (collection.method !== 'cash' || collection.remittance_status === 'paid')); return <article key={delivery.delivery_id} className="card overflow-hidden">

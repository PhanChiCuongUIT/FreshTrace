...
type Answer = { answer: string; intent: string; recommendations: Recommendation[] }
...
export function AssistantPage() {
  ...
  return <div><PageHeader eyebrow="Decision support" title="Fresh Assistant" />
    <div className="card mt-6 p-6"><div className="flex gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-brand-100 text-brand-700"><Bot /></div><div><h2 className="font-black">Ask using natural language</h2><p className="text-sm text-black/50">Recommendations only use products currently available in FreshTrace.</p></div></div>
      <form className="mt-5 flex flex-col gap-3 sm:flex-row" onSubmit={event => { event.preventDefault(); ask() }}><input className="input" minLength={3} required placeholder="Show the cheapest products or products expiring soon" value={question} onChange={event => setQuestion(event.target.value)} /><button disabled={loading} className="btn-primary">{loading ? 'Thinking...' : 'Ask'}</button></form>
      <div className="mt-3 flex flex-wrap gap-2">{['Cheapest rice', 'Fresh meat', 'Certified vegetables', 'Fruit with long shelf life', 'Mushrooms expiring soon', 'Best Fresh Rescue deals'].map(value => <button key={value} type="button" className="rounded-full bg-black/[0.04] px-3 py-1.5 text-xs font-semibold hover:bg-brand-50 hover:text-brand-700" onClick={() => setQuestion(value)}>{value}</button>)}</div>
    </div>
    {answer && <div className="mt-6"><p className="mb-4 rounded-xl bg-brand-50 p-4"><b>Intent: {answer.intent.replaceAll('_', ' ')}</b><br/>{answer.answer}</p><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{answer.recommendations.map(item => <article key={`${item.productId}:${item.batchId}`} className="card overflow-hidden"><Link to={`/products/${item.productId}`} className="block h-40 bg-gradient-to-br from-brand-50 to-orange-50">{item.imageUrl ? <img src={item.imageUrl} alt={item.name} className="h-full w-full object-cover"/> : <div className="grid h-full place-items-center text-brand-700"><Bot size={38}/></div>}</Link><div className="p-5"><div className="flex justify-between gap-3"><Link to={`/products/${item.productId}`} className="font-black hover:text-brand-700">{item.name}</Link>{item.isRescue && <span className="text-sm font-bold text-orange-600">-{item.rescueDiscountPercent}%</span>}</div><p className="text-sm text-black/50">{item.category} / expires {date(item.expireDate)}</p><p className="mt-3 text-lg font-black">{currency.format(item.currentPrice)} / {item.unit}</p><p className="text-sm">{item.certificate ?? 'No certificate listed'}</p><div className="mt-4 grid grid-cols-2 gap-2"><Link className="btn-secondary py-2" to={`/products/${item.productId}`}>View details</Link><button className="btn-primary py-2" onClick={() => addToCart(item)}><ShoppingCart size={16}/> Add</button></div></div></article>)}</div></div>}
  </div>
}

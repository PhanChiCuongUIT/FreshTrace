...
export function ReportsPage() {
  ...
  const data = useQuery({ queryKey: ['customer-reports', profile?.user_id], queryFn: async () => {
    const [reports, orders] = await Promise.all([
      supabase.from('reports').select('report_id,type,description,status,response,created_at,orders(order_code)').eq('user_id', profile!.user_id).order('created_at', { ascending: false }),
  ...
  return <div>
    ...
    <form className="card mt-6 grid gap-4 p-5" onSubmit={event => { event.preventDefault(); void submit() }}>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <label className="text-sm font-bold">Type<select className="input mt-1" value={type} onChange={event => setType(event.target.value)}><option value="order_issue">Order issue</option><option value="quality_issue">Product quality</option><option value="delivery_issue">Delivery issue</option><option value="payment_issue">Payment issue</option><option value="other">Other</option></select></label>
    ...
    <section className="mt-7">
      ...
      <div className="mt-4 space-y-3">{!data.data?.reports.length ? <EmptyState title="No reports yet" /> : data.data.reports.map(report => <article key={report.report_id} className="card p-5">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><b className="capitalize">{report.type.replaceAll('_', ' ')}</b><p className="text-sm text-black/45">{report.orders ? `Order #${report.orders.order_code} / ` : ''}{dateTime(report.created_at)}</p></div><Badge tone={report.status === 'resolved' ? 'green' : report.status === 'rejected' ? 'red' : 'orange'}>{report.status}</Badge></div>

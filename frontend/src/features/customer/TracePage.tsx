...
export function TracePage() {
  ...
  return <div><PageHeader eyebrow="Transparency" title="Trace a food batch" />
    <form className="card mt-4 grid gap-3 p-4 sm:grid-cols-[1fr_auto] lg:mt-6" onSubmit={event => { event.preventDefault(); trace() }}><input className="input" required placeholder="Enter a batch code" value={code} onChange={event => setCode(event.target.value)} /><div className="flex gap-2"><div className="lg:hidden"><QrScanner onResult={value => trace(value)}/></div><button className="btn-primary flex-1">Trace</button></div></form>
